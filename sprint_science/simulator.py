...
class SprintSimulation:
    ...
    def segments(self) -> pd.DataFrame:
        ...
        for record in boundary_list:
            ...
            segment = {"distance": record, "total_time": total_time, "segment_time": segment_time}
    ...
    def flying_sections(self) -> dict[str, dict[str, float]] | str:
        """
        ...
        if self.fly_length > 60:
            raise ValueError("Maximum supported fly length is 60m.")

        elif self.running_distance < 100:
            raise ValueError("To perform the flying section calculation, please enter a run distance of at least 100m.")
    ...
    def f_v_profile_comparison(self) -> pd.DataFrame:
        ...
        for value in f_v_slopes_range:
            ...
            temp_simulation = SprintSimulation(
                F0=F0_new,
                V0=V0_new,
                weight=self.weight,
                height=self.height,
            ...
            current_f_v = {"f_v_slope": value, "time": data["time"].iloc[-1], "F0": F0_new, "V0": V0_new}
    ...
    def overspeed_zones(self) -> pd.DataFrame:
        ...
        # Baseline (unloaded)
        baseline_sim = SprintSimulation(
            F0=self.F0, V0=self.V0, weight=self.weight, height=self.height, running_distance=100, external_force_N=0
        )
        ...
        for i in range(1, 200):
            ...
            overspeed_zones.append(
                {"external_force_N": external_force_N, "top_speed": top_speed, "speed_gain": speed_gain}
            )

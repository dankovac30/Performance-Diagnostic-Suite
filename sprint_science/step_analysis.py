...
class StepAnalyzer:
    ...
    def prepare_dataframe(self) -> pd.DataFrame:
        ...
        if self.spatiotemporal_data is None:
            ...
            self.spatiotemporal_data['air_drag'] = calculate_air_resistance_force(self.spatiotemporal_data['speed'], self.air_density, self.frontal_area, self.wind_speed)
            
            # Total force = Inertial force + machine resistance + air resistance
            self.spatiotemporal_data['total_force'] = self.spatiotemporal_data['force'] + self.spatiotemporal_data['external_force'] + self.spatiotemporal_data['air_drag']
            ...
            smooth_speed = apply_butterworth_filter(self.spatiotemporal_data, data_to_smooth='speed', cutoff_freq=1.3, order=4)
    ...
    def analyze_steps(self) -> pd.DataFrame:
        ...
        for start, end in zip(cross_indices, cross_indices[1:]):
            ...
            else: braking_propulsive_ratio = 0

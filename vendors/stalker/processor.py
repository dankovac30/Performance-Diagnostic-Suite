...
from vendors.stalker.preprocessing import crop_start, reconstruct_start, crop_end
from core.signal_processing import apply_median_filter, apply_butterworth_filter, apply_corridor_filter
import numpy as np
import pandas as pd
...
def process_rda_file(file_path: str, sampling_rate: float = 46.875) -> pd.DataFrame:
    ...
    reconstructed_df['smooth_distance'] = (reconstructed_df['smooth_speed'].rolling(window=2).mean() * dt).fillna(0).cumsum()
